...
It takes the same options as the macOS overlay (below). `--scale` is in points (default 5) and follows your display scaling. Terminals `--track-terminal` recognizes: Windows Terminal, conhost (cmd/PowerShell), WezTerm, Alacritty, mintty (Git Bash), Hyper, Tabby, Warp, Ghostty. Only one overlay runs at a time; a second launch exits immediately.
